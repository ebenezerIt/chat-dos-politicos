...
export class PoliticosRepository {
  constructor(
    private politicosClient: PoliticosClient,
    private mapper: PoliticosMapper,
    private localStorage: LocalStorage
  ) {}

  listParliamentarians(): Observable<ParliamentarianListResponse> {
    const localParliamentariansResponse: ParliamentarianListResponse =
      this.localStorage.get('listParliamentarians');
    const observableReturn = new Subject<ParliamentarianListResponse>();

    if (
      localParliamentariansResponse &&
      !this.localStorage.isLocalStorageExpired()
    ) {
      ...
      this.shrinkParliamentariansList(listResponseObservable).subscribe(
        parliamentarians => {
  ...
  getParliamentarianVotesById(
    id: number
  ): Observable<ParliamentarianSingleResponse> {
  ...
  getExpenditureById(id: number): Observable<Expenditure> {
    ...
    this.politicosClient
      .getParliamentarianQuotasById(id)
      .subscribe(parliamentarianSingleResponse => {
        const expense = this.mapper.toExpenditure(
          parliamentarianSingleResponse
        );
  ...
  shrinkParliamentariansList(
    listResponseObservable: Observable<ParliamentarianListResponse>
  ): Observable<ParliamentarianListResponse> {
    return listResponseObservable.pipe(
      map((parliamentarian: ParliamentarianListResponse) =>
        this.mapper.shrinkParliamentarians(parliamentarian)
      )
    );
  }

  private fixParliamentariansPosition(
    parliamentarians: ParliamentarianListResponse
  ): void {
    let position = 1;
