...
export class SidebarComponent {
  ...
  get filteredConversations(): ParliamentarianDataResponse[] {
    ...
    list.sort((a, b) => {
      ...
      return a.parliamentarian.nickname.localeCompare(
        b.parliamentarian.nickname,
        'pt-BR',
        { caseFirst: 'upper' }
      );
    });
  ...
  handleConversationClicked(conversation: ParliamentarianDataResponse): void {
    ...
    this.politicosService
      .getParliamentarianVotesById(conversation.parliamentarianId)
  ...
  getProgressBar(resumes) {
    ...
    if (newResumes.length > 3) {
      return [
        ...newResumes.filter(resume => resume.color != 'orange'),
        ...this.returnNoVotes(
          newResumes.filter(resume => resume.color === 'orange')
        ),
      ];
